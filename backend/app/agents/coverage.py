...
from app.agents.serializers import coverage_snapshot, evidence_refs_to_json
...
def _module_key_for_id(db: Session, module_id: str | None) -> str:
    if not module_id:
        return "UNMAPPED"
...
def lookup_coverage_records(
    ...
    *,
    ...
    def append(record: dict[str, Any]) -> None:
        ...
        haystack = " ".join(str(record.get(key) or "") for key in ("title", "behavior_summary", "expected_result"))
        ...
        append(
            {
                ...
                "module_key": _module_key_for_id(db, module_id),
                "title": title,
    ...
    if query_tokens:
        records.sort(key=lambda item: jaccard(query_tokens, set(item.get("tokens", []))), reverse=True)
    return records[:max_results]


def classify_candidate_coverage(candidate: Any, records: list[dict[str, Any]]) -> dict[str, Any]:
...
def transition_staged_output_coverage(
    db: Session,
    *,
    output: AgentStagedOutput,
    decision_status: AgentStagedOutputStatus | str,
    changed_at: datetime,
) -> tuple[str, list[CoverageIndexEntry]]:
    status_value = decision_status.value if isinstance(decision_status, AgentStagedOutputStatus) else decision_status
    if status_value == AgentStagedOutputStatus.accepted.value:
        next_coverage_state = "candidate"
        verified_by_human = True
    elif status_value == AgentStagedOutputStatus.rejected.value:
        next_coverage_state = "rejected"
        verified_by_human = False
    else:
        raise ValueError("Coverage transition only supports accepted or rejected staged outputs")
    ...
    for entry in coverage_entries:
        ...
        entry.verified_by_human = verified_by_human
    output.coverage_entries = [coverage_snapshot(entry) for entry in coverage_entries]
    return next_coverage_state, coverage_entries

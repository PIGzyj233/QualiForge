...
logger = logging.getLogger("qualiforge.agent_worker")
...
async def run_heartbeat(settings) -> None:
    while True:
        ...
        try:
            ttl_seconds = settings.worker_heartbeat_seconds * 3
            await client.set(AGENT_WORKER_HEARTBEAT_KEY, "ok", ex=ttl_seconds)
            await client.set(LEGACY_WORKER_HEARTBEAT_KEY, "ok", ex=ttl_seconds)
            logger.info("Agent worker heartbeat recorded")
        except Exception:
...
async def run_temporal_worker(settings) -> None:
    logger.info("Starting Agent Temporal worker on %s queue %s", settings.temporal_address, settings.agent_task_queue)
